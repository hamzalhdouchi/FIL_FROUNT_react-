...
const fetchCommandesLivre = useCallback(async () => {
    setIsLoading(true)
    setError(null)
    
    try {
    const response = await axios.get(
        `${API_BASE_URL}/commandes/restaurant/${restaurant_id}`,
    )
    console.log(response);
    
    const allCommandes = response.data.data 
    const Commandes = allCommandes.filter(cmd => cmd.CommandStatus === 'livraison');
    setCommandes({
        [STATUS_TYPES.EN_ATTENTE]: Commandes.filter(cmd => cmd.statut === STATUS_TYPES.EN_ATTENTE),
        [STATUS_TYPES.EN_COURS]: Commandes.filter(cmd => cmd.statut === STATUS_TYPES.EN_COURS),
        [STATUS_TYPES.TERMINEE]: Commandes.filter(cmd => cmd.statut === STATUS_TYPES.TERMINEE),
        [STATUS_TYPES.ANNULEE]: Commandes.filter(cmd => cmd.statut === STATUS_TYPES.ANNULEE),
    })
    } catch (error) {
...
useEffect(() => {
    fetchCommandesLivre()
    fetchPayments()
}, [fetchCommandesLivre, fetchPayments])
...
const handleAction = useCallback(async (action, commande, additionalData = {}) => {
    ...
    switch (action) {
        ...
        if (!token) {
            const url = `http://localhost:3000/commandes/${restaurant_id}`
    ...
    await fetchCommandesLivre()
    return response
    } catch (error) {
    console.error(`${action} error:`, error)
    throw error
    }
}, [restaurant_id, navigate, fetchCommandesLivre])
...
const tabs = useMemo(() => [
    ...
    { id: STATUS_TYPES.TERMINEE, label: "Livrée" },
...
return (
    ...
    <div className="absolute top-10 left-20">
        <button 
        onClick={() => navigate(-1)}
        className="text-wood-600 hover:text-wood-800 transition-colors duration-200 flex items-center group"
        >
        <i className="bx bx-chevron-left text-2xl mr-1 group-hover:-translate-x-1 transition-transform duration-200"></i>
    ...
    <div className="container mx-auto px-4 text-center">
        ...
        Suivez vos commandes de livraison et gérez-les facilement.
        </p>
    </div>
    </header>
...
export default CommandesPageLivr